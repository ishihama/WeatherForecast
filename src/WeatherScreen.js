...
type Props = {
    navigations: NavigationScreenProps<*>,
};
...
class WeatherScreen extends Component<Props, State> {
    ...
    componentDidMount() {
        ...
        const { city } = navigation.state.params;
        getCurrentWeather(city).then(current => this.setState({current}));
        getWeatherForecast(city).then(forecasts => this.setState({forecasts}));
    }
